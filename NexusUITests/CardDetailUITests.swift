...
final class CardDetailUITests: XCTestCase {
    ...
    func testFreezeRoundTripReflectsOnDetailAndDashboard() {
        ...
        XCTAssertTrue(freezeButton.waitForExistence(timeout: UITestTimeout.seconds(10)))
        ...
        XCTAssertTrue(confirm.waitForExistence(timeout: UITestTimeout.seconds(10)))
        confirm.tap()

        XCTAssertTrue(waitForLabel(containing: "Frozen", on: status))
        XCTAssertTrue(freezeButton.waitForNonExistence(timeout: UITestTimeout.seconds(10)))
        ...
        XCTAssertTrue(statusAfterReload.waitForExistence(timeout: UITestTimeout.seconds(10)))
    ...
    func testFreezeFailureLeavesTheCardActive() {
        ...
        XCTAssertTrue(alertHeadline.waitForExistence(timeout: UITestTimeout.seconds(20)))
        // …dismissing keeps the card active and the controls usable.
        app.alerts.buttons["OK"].tap()
        XCTAssertTrue(status.label.contains("Active"), "label was: \(status.label)")
        XCTAssertTrue(app.buttons[CardDetailAccessibility.freeze].waitForExistence(timeout: UITestTimeout.seconds(10)))
    }
    ...
    private func backToDashboard(from app: XCUIApplication) {
        ...
        XCTAssertTrue(backButton.waitForExistence(timeout: UITestTimeout.seconds(10)))
        backButton.tap()
    }
    ...
    private func waitForLabel(containing text: String, on element: XCUIElement) -> Bool {
        ...
        return XCTWaiter.wait(for: [expectation], timeout: UITestTimeout.seconds(20)) == .completed
    }
}
